//! Domain policy for Expense aggregate
//!
...
/// Domain policy for Expense — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Expense>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ExpenseDomainPolicy = PermitAllPolicy<Expense>;
