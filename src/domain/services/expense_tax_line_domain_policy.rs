//! Domain policy for ExpenseTaxLine aggregate
//!
...
/// Domain policy for ExpenseTaxLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ExpenseTaxLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ExpenseTaxLineDomainPolicy = PermitAllPolicy<ExpenseTaxLine>;
