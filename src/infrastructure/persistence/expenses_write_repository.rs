...
//! connection — the service relayed the ambient request org scope onto it
//! (`org_scope::bind_org_scope_on`) — so a decorated deployment's row-level fences scope
//! every statement. The module itself carries no tenant predicate in any statement
//! (ADR-0029): an undecorated deployment gets an unfenced module.
//!
//! Soft-delete lives in `metadata` JSONB (`deleted_at` key), so every "live row" predicate is
//! `(metadata->>'deleted_at') IS NULL`, mirroring the module's partial indexes. The
//! `(approval_state, state)` table CHECK (`expenses_state_pair_legal`) is the arbiter of
...
impl ExpensesWriteRepository {
    ...
    /// The live category (create/post validation).
    ...
    /// The INSERT never names an org column — a decorator-installed `org_unit_id`
    /// carries its own acting-unit DEFAULT.
    pub async fn insert_expense(
        ...
        sqlx::query_as::<_, Expense>(
            r#"INSERT INTO expenses.expenses
               ...
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8::expense_payment_mode, $9,
                       'draft', 'draft', $10,
                       jsonb_build_object('created_by', to_jsonb($11::uuid), 'created_at', to_jsonb($12::timestamptz)))
               RETURNING *"#,
        )
        .bind(expense.id)
    ...
    pub async fn update_expense(
        ...
        sqlx::query_as::<_, Expense>(
            r#"UPDATE expenses.expenses SET
                   ...
                   reference      = CASE WHEN $8 THEN $9 ELSE reference END,
                   metadata       = metadata || jsonb_build_object(
                                       'updated_by', to_jsonb($10::uuid),
    ...
    pub async fn mark_submitted(
        ...
        sqlx::query_as::<_, Expense>(
            r#"UPDATE expenses.expenses SET
                   ...
                   metadata = metadata || jsonb_build_object(
                       'submitted_by', to_jsonb($3::uuid),
                       'updated_at', to_jsonb($4::timestamptz))
               WHERE id = $1
                 AND approval_state = 'draft' AND state = 'draft'
                 AND employee_id = $5 AND category_id = $6
                 AND expense_date = $7 AND amount_total = $8 AND currency = $9
                 AND (metadata->>'deleted_at') IS NULL
               RETURNING *"#,
        )
        .bind(expense_id)
    ...
    pub async fn mark_refused(
        ...
        sqlx::query_as::<_, Expense>(
            r#"UPDATE expenses.expenses SET
                   ...
                   state = 'refused',
                   metadata = metadata || jsonb_build_object(
                       'refused_by', to_jsonb($2::uuid),
                       'refused_reason', to_jsonb($3::text),
                       'updated_at', to_jsonb($4::timestamptz))
               WHERE id = $1
                 AND approval_state = 'submitted' AND state = 'submitted'
                 AND (metadata->>'deleted_at') IS NULL
    ...
    pub async fn mark_posted(
        ...
        sqlx::query_as::<_, Expense>(
            r#"UPDATE expenses.expenses SET
                   state = 'posted',
                   journal_id = $2,
                   accounting_post_id = $3,
                   metadata = metadata || jsonb_build_object(
                       'posted_by', to_jsonb($4::uuid),
    ...
    pub async fn replace_tax_lines(
        ...
        sqlx::query(
            r#"UPDATE expenses.expense_tax_lines SET
                   metadata = metadata || jsonb_build_object('deleted_at', to_jsonb($2::timestamptz))
               WHERE expense_id = $1
                 AND (metadata->>'deleted_at') IS NULL
        ...
        for line in lines {
            let n = sqlx::query(
                r#"INSERT INTO expenses.expense_tax_lines
                       (id, expense_id, basis, account_id, description, rate, tax_amount, metadata)
                   SELECT $1, $2, $3, $4, $5, $6, $7,
                          jsonb_build_object('created_by', to_jsonb($8::uuid), 'created_at', to_jsonb($9::timestamptz))
    ...
    /// caller's scope-relayed connection: a decorated deployment's row-level fences govern
    /// which rows the projection sees; there is no write surface at all.
    pub async fn report(
        &self,
        conn: &mut sqlx::PgConnection,
